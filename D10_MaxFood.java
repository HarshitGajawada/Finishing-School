...
public class D10_MaxFood{
    private static double dist(int[] home, int i, int j){
        return Math.sqrt(((home[0]-i)*(home[0]-i))+((home[1]-j)*(home[1]-j)));
    }
    private static void backtrack(double currt, int cnt, int r, int c, int[][] food, int[][] nests, int[] home, boolean[] vis, int[] res){
        res[0]=Math.max(res[0],cnt);
        for(int i=0;i<food.length;i++){
            if(!vis[i]){
                vis[i]=true;
                double toFood=dist(food[i],r,c);
                for(int j=0;j<nests.length;j++){
                    double toNest=dist(nests[j],food[i][0],food[i][1]);
                    double toHome=dist(home,nests[j][0],nests[j][1]);
                    if(currt==toFood+toNest+toHome){
                        backtrack(currt-(toFood+toNest+toHome),cnt+1,nests[j][0],nests[j][1],food,nests,home,vis,res);
                    }
                    else if(currt>toFood+toNest+toHome){
                        backtrack(currt-(toFood+toNest),cnt+1,nests[j][0],nests[j][1],food,nests,home,vis,res);
                    }
                }
                vis[i]=false;
            }
    ...
    public static void main(String[] args){
        ...
        boolean[] vis=new boolean[nf];
        int[] res=new int[1];
        backtrack(time,0,home[0],home[1],food,nests,home,vis,res);
