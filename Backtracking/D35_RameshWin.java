...
public class D35_RameshWin {
    private static boolean backtrack(String s, boolean[] vis){
        for(int i=0;i<s.length()-1;i++){
            if(s.charAt(i)=='A' && s.charAt(i+1)=='A' && !vis[i] && !vis[i+1]){
                vis[i]=true;
                vis[i+1]=true;
                boolean lose=!backtrack(s,vis);
                vis[i]=false;
                vis[i+1]=false;
                if(lose){
                    return true;
    ...
    public static void main(String[] args){
        ...
        System.out.println(backtrack(s,vis));
        sc.close();
    }
}
