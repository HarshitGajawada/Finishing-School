...
public class D24_FindSum {
    private static boolean find(TreeNode r, int req){
        if(r==null){
            return false;
        }
        if(r.val==req){
        ...
        if(req<r.val){
            return find(r.left,req);
        }
        else{
            return find(r.right,req);
        }
    }
    public boolean twoSumBSTs(TreeNode root1, TreeNode root2, int target) {
        ...
        if(find(root2,Math.abs(root1.val-target))){
            return true;
        }
