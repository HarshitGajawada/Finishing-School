...
public class D87_UniqueName {
    public static void main(String[] args){
        ...
        while(i<tn.length() && j<mn.length()){
            if(tn.substring(i).compareTo(mn.substring(j))>0){
                res.append(tn.charAt(i++));
            }
            else{
                res.append(mn.charAt(j++));
            }
        }
        if(i<tn.length()){
            res.append(tn.substring(i));
        }
        if(j<mn.length()){
            res.append(mn.substring(j));
        }
