...
public class D38_TombsRob{
    ...
    public static int getTombs(int[] artifacts){
        ...
        int skipfirst=check(artifacts,1,artifacts.length-1,new int[artifacts.length]);
        int skiplast=check(artifacts,0,artifacts.length-2,new int[artifacts.length-1]);
        return Math.max(skipfirst,skiplast);
    }
    public static void main(String[] args){
        Scanner sc=new Scanner(System.in);
        String[] inp=sc.nextLine().split(" ");
        int[] artifacts=new int[inp.length];
        for(int i=0;i<inp.length;i++){
            artifacts[i]=Integer.parseInt(inp[i]);
        }
