...
public class D45_MaxPick{
    public static void main(String[] args){
        ...
        for(int i=0;i<n;i++){
            board[i]=sc.nextInt();
            f.put(board[i],f.getOrDefault(board[i],0)+1);
        }
        int len=f.size();
        ...
        Arrays.sort(vals);
        int[] dp=new int[len];
        dp[0]=vals[0]*f.get(vals[0]);
        if(vals[1]-vals[0]>1){
            dp[1]=vals[1]*f.get(vals[1])+dp[0];
        }
        else{
            dp[1]=Math.max(vals[1]*f.get(vals[1]),dp[0]);
        }
        for(int i=2;i<len;i++){
            if(vals[i]-vals[i-1]>1){
                dp[i]=vals[i]*f.get(vals[i])+dp[i-1];
            }
            else{
                dp[i]=Math.max(dp[i-1],dp[i-2]+vals[i]*f.get(vals[i]));
            }
        }
